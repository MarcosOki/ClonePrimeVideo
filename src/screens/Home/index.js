    ...
    return(
        <View style={styles.container}>
            ...
            <View style={styles.continueWatching}>
                ...
                <FlatList  horizontal={true} style={styles.list}  contentContainerStyle={{gap:15}} data={MOVIESWATCHING} keyExtractor={(item) => item.id} renderItem={({item})=>(<CardMovie url={item.moviesURL} />)}/>
            </View>
            <View style={styles.continueWatching}>
                <View style={styles.boxTextMovies}>
                    <Text style={styles.btnText}>Crimes Movies</Text>
                </View>
                <FlatList  horizontal={true} style={styles.list}  contentContainerStyle={{gap:15}} data={MOVIESCRIME} keyExtractor={(item) => item.id} renderItem={({item})=>(<CardMovie url={item.moviesURL} />)}/>
            </View>
            <View style={styles.continueWatching}>
                <View style={styles.boxTextMovies}>
                    <Text style={styles.btnText}>Assista em seu idioma</Text>
                </View>
                <FlatList  horizontal={true} style={styles.list}  contentContainerStyle={{gap:15}} data={MOVIESWATCH} keyExtractor={(item) => item.id} renderItem={({item})=>(<CardMovie url={item.moviesURL} />)}/>
            </View>
        </View>
    )
}
